# def _get_reward(self, action_list, closing_price, opening_price, buy_reward, sell_reward, money_change):
#     #reward = self.portfolio_value - self.initial_funds
...
#         ratios.append(opening_price/closing_price)

#     for ii, action in enumerate(action_list):
#         reward += (money_change[ii] * action * (closing_price[ii] - opening_price[ii])/closing_price[ii])#/self.previous_portfolio

#         #Penalise if selling when the closing price is higher than the opening price
#         '''Future Idea: Remove penalty if selling stock while buying other stock and increase is more'''
...
#     reward += (buy_reward + sell_reward)#/self.previous_portfolio
...
"""def _get_reward(self, action_list, closing_price, opening_price, buy_reward, sell_reward, money_change):
    ...
    #Adds reward for buying before a price goes up and selling before a price goes down
    for ii, action in enumerate(action_list):
        ratios.append(opening_price[ii]/closing_price[ii])
        penalties.append((closing_price[ii] - opening_price[ii])/closing_price[ii])
    ...
    for ii, action in enumerate(action_list):
        ...
        '''Future Idea: Remove penalty if selling stock while buying other stock and increase is more'''
        #Penalise if selling when the closing price is higher than the opening price unless buying other stock with higher increase in price
        if closing_price[ii] > opening_price[ii] and action < 0:
            for jj in range(len(ratios)):
                if ratios[ii] < ratios[jj] and action_list[jj] > 0: #If ratio of current stock increase is smaller than other ratio and action on other stock is buy
                    pass
                else:
                    reward -= abs(action) * penalties[ii]

    #Penalty for selling or buying stock when action is not available
    reward += (buy_reward + sell_reward)#/self.previous_portfolio

    #Add reward for having higher portfolio value than initial funds
    reward += (self.portfolio_value - self.initial_funds)/self.initial_funds
...
def _get_observation_julian_0(self):
    ...
    opening_price_prev = np.array([self.stock_data.loc[(
        ticker, prev_date), "Open"] for ticker in self.ticker_list])
