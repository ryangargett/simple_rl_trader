...
class EricTrader(SimpleTrader):
    ...
    def _get_reward_eric_0(self):
        ...
        profit_reward = self.portfolio_value - self.initial_funds
        ...
        portfolio_return = ((self.portfolio_value - self.initial_funds)/self.initial_funds) * 100
        if portfolio_return > 3:                       #small reward for performing good
            min_reward = 10 * (portfolio_return - 3) 
        elif portfolio_return < 3:                     #larger penalisation for bad performance
            min_reward = -(15 * (3 - portfolio_return))            
        elif portfolio_return < 0:                        #negative performance = large penalisation
            min_reward = 50 * portfolio_return

        #has to make a 3% improve from the previous portfolio
        #calculate percentage return scale: x% value
        return_per = ((self.portfolio_value - self.previous_portfolio)/self.previous_portfolio) * 100
        print(colored(f'previous return: {return_per}', 'blue')) 
        print(colored(f'percentage return: {min_reward}', 'blue'))

        if return_per > 3:                       #small reward for performing good
            return_reward = 10 * (return_per - 3) 
        elif return_per < 3:                     #larger penalisation for bad performance
            return_reward = -(15 * (3 - return_per))            
        elif return_per < 0:                        #negative performance = large penalisation
            return_reward = 50 * return_per

        reward = funds_penalty + profit_reward + portfolio_reward + return_reward + portfolio_return + min_reward
        
        print(colored(f'funds penalty {funds_penalty}', 'magenta'))
        print(colored(f'portfolio % return {min_reward}', 'magenta'))
