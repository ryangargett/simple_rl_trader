...
class EricTrader(SimpleTrader):
    ...
    def _get_reward_eric_0(self):
        ...
        #setting a minimum performance return it has to make (4%) will get reward for every percent above
        #4% and penalise if it doesnt

        #calculate percentage return
        return_per = (self.portfolio_value - self.previous_portfolio) - 1
        if return_per > 0.04:
            return_reward = 10 * (return_per - 0.04) #small reward for performing good
        elif return_per < 0.04:
            return_reward = 15 * (0.04 - return_per)            #larger penalisation for bad performance
